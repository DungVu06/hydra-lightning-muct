...
class MUCTLitModule(LightningModule):
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        # update and log metrics
        self.val_loss(loss)
        self.log("val/nme", batch_nme, on_step=False, on_epoch=True, prog_bar=True)
        self.log("val/loss", self.val_loss, on_step=False, on_epoch=True, prog_bar=True)

    def on_validation_epoch_end(self) -> None:
        "Lightning hook that is called when a validation epoch ends."
        current_val_loss = self.val_loss.compute()  # get current val loss
        self.val_loss_best(current_val_loss)  # update best so far val loss
